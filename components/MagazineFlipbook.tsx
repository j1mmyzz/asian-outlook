...
export default function MagazineFlipbook({ pdfUrl }: { pdfUrl: string }) {
  ...
  const pageWidth = isMobile ? MOBILE_MAX_PAGE_WIDTH : DESKTOP_MAX_PAGE_WIDTH;
  ...
  return (
    <div className="w-full py-8">
      ...
      <div className="flex w-full justify-center overflow-x-auto px-4">
        <Document
          file={pdfUrl}
        ...
        >
          {numPages > 0 && (
            <HTMLFlipBook
              key={`${pageWidth}-${pageHeight}-${isMobile}`}
              ref={flipBookRef}
              width={pageWidth}
              height={pageHeight}
              minWidth={pageWidth}
